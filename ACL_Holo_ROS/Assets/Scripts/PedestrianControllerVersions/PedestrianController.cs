...
public class PedestrianController : MonoBehaviour
{
    ...
    private bool mappingFinished = false;   // callback from SpatialMappingManager
    ...
    private void CreatePedestrians()
    {
        for (int i = 0; i < numPedestrians; i++)
        {
          ...
          //  tempPedestrian.transform.SetParent(OriginCube.transform);  // Make sure this is not null. Sets position relative to origin. Could just be transform of OriginCube, but this method works if originCube 
    ...
    private void HandlePosesUpdated(geometry_msgs.PoseArray poses)
    {
        ...
        //if (firstSignal && mappingFinished)
        //{
        //    Debug.Log("First Pose Aquired");
        //    poseStatus = -1; // changes from -2 to -1 if first time and mapping finished
        //    numPedestrians = poses.Count;
        //    firstSignal = false;  // Creating pedestrians not allowed in threads other than main, so we must wait for the Update function (No coroutines allowed).
        //} else {
        //    poseStatus = 1;
        //}
